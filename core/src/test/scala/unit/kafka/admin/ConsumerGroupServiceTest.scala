...
class ConsumerGroupServiceTest {
  ...
  def testAdminRequestsForDescribeOffsets(): Unit = {
    ...
    when(admin.listConsumerGroupOffsets(ArgumentMatchers.eq(Collections.singletonMap(group, ListConsumerGroupOffsetsOptions.ALL_TOPIC_PARTITIONS)), any()))
    ...
    verify(admin, times(1)).listConsumerGroupOffsets(ArgumentMatchers.eq(Collections.singletonMap(group, ListConsumerGroupOffsetsOptions.ALL_TOPIC_PARTITIONS)), any())
